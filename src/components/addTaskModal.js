...
function AddTaskModal({ closeModal }) {
  ...
  const isTaskEmpty = taskTitle.trim() === '';
        ...
        <form>
          <div className="add-edit-modal">
            ...
            <div className="modal-priority">
              <div className="modal-priority">
                ...
                <ul className="priority-buttons">
                  <li className={`high${taskPriority === 'high' ? '-selected' : ''}`}
                    onClick={() => setTaskPriority('high')}>
                    high
                  </li>
                  <li className={`medium${taskPriority === 'medium' ? '-selected' : ''}`}
                    onClick={() => setTaskPriority('medium')}>
                    medium
                  </li>
                  <li className={`low${taskPriority === 'low' ? '-selected' : ''}`}
            ...
            <div className="flx-right mt-50">
              <button className="button" onClick={handleAddTask} disabled={isTaskEmpty}>
                Add
              </button>
            </div>
          </div>
