...
function TaskCard() {
  ...
  useEffect(() => {
    setStatuses(tasks.map((task) => task.status));
  }, [tasks]);
  ...
  const handleStatus = (index) => {
    ...
    const updatedTask = { ...tasks[index], status: updatedStatus };
    dispatch(editTask({ id: tasks[index].id, editedTask: updatedTask }));
  };
  ...
  const handleDelete = () => {
    dispatch(deleteTask(tasks[deleteTaskIndex].id));
    setShowDelete(false);
  };
